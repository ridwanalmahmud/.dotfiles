    ...
    config = function()
        ...
        ls.add_snippets("make", {
            s("makefile_c", {
                ...
                t({ "", "CFLAGS := -std=c99  -Wall -Wextra -I$(INCDIR) -pedantic -D_GNU_SOURCE \\" }),
                ...
                priority = 100,
                description = "Basic Makefile for c",
            ...
            s("makefile_c++", {
                ...
                t({ "", "# Avoid in release flags -> -g3 -fsanitize=address" }),
                t({ "", "# Avoid in development flags -> -O2 -march=native" }),
                t({ "", "CFLAGS := -std=c++17 -Wall -Wextra -I$(INCDIR) -pedantic -D_GNU_SOURCE \\" }),
                t({ "", "\t   -D_FORTIFY_SOURCE=2 -fstack-protector-all -fPIE -fPIC -MMD -MP \\" }),
                t({ "", "\t   $(OPT) -march=native \\" }),
                t({ "", "\t   -g3 -fsanitize=address" }),
                t({ "", "LDFLAGS := -pie" }),
                ...
                i(1, "gtest"),
                t({ "", "" }),
                t({ "", "SRCS := $(wildcard $(SRCDIR)/*.cpp)" }),
                t({ "", "OBJS := $(patsubst $(SRCDIR)/%.cpp, $(OBJDIR)/%.o, $(SRCS))" }),
                t({ "", "DEPS := $(patsubst $(SRCDIR)/%.cpp, $(OBJDIR)/%.d, $(SRCS))" }),
                t({ "", "EXEC := $(BUILDDIR)/" }),
                i(2, "main"),
                t({ "", "" }),
                t({ "", "TESTSRCS := $(wildcard $(TESTDIR)/*.cpp)" }),
                t({ "", "TESTOBJS := $(patsubst $(TESTDIR)/%.cpp, $(TESTOBJDIR)/%.o, $(TESTSRCS))" }),
                t({ "", "TESTBINS := $(patsubst $(TESTOBJDIR)/%.o, $(TESTBUILDDIR)/%, $(TESTOBJS))" }),
                t({ "", "TESTDEPS := $(patsubst $(TESTDIR)/%.cpp, $(TESTOBJDIR)/%.d, $(TESTSRCS))" }),
                t({ "", "" }),
                t({ "", "-include $(DEPS)" }),
                t({ "", "-include $(TESTDEPS)" }),
                t({ "", "" }),
                ...
                t({ "", "\t$(CC) $(CFLAGS) $(LDFLAGS) $< $(filter-out $(OBJDIR)/main.o, $(OBJS)) -o $@ $(TESTLIB)" }),
                t({ "", "" }),
                t({ "", "$(TESTOBJDIR)/%.o: $(TESTDIR)/%.cpp | $(TESTOBJDIR)" }),
                t({ "", "\t$(CC) $(CFLAGS) -c $< -o $@" }),
                t({ "", "" }),
                t({ "", "$(BUILDDIR) $(OBJDIR) $(TESTBUILDDIR) $(TESTOBJDIR):" }),
                t({ "", "\tmkdir -p $@" }),
